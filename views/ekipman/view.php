...
<div class="ekipman-view">
    ...
    <ul class="nav nav-tabs" id="ekipmanTab" role="tablist">
        ...
        <?php if (!empty($analizorConfig)): ?>
        <li class="nav-item">
            <a class="nav-link" id="enerji-tab" data-toggle="tab" href="#enerji" role="tab">
                <span style="color:#e94560">⚡</span> Enerji
            </a>
        </li>
        <?php endif; ?>
    </ul>
    ...
    <div class="tab-content" id="ekipmanTabContent">
       ...
       <?php if (!empty($analizorConfig)): ?>
       <div class="tab-pane fade" id="enerji" role="tabpanel">
            <div class="mt-3">
                <div class="d-flex align-items-center mb-3">
                    <h5 class="mb-0" style="color:#e94560">⚡ Enerji Analizörü — <?= Html::encode($analizorConfig['model']) ?></h5>
                    ...
                    <button class="btn btn-sm btn-outline-success ml-auto" id="analizorStartBtn" onclick="analizorToggle()">
                        ▶ Başlat
                    </button>
                </div>
                <?php if (!Yii::$app->user->isGuest && Yii::$app->user->identity->role === 'admin'): ?>
                <div class="text-muted small mb-2">
                    IP: <?= Html::encode($analizorConfig['ip']) ?>:<?= Html::encode($analizorConfig['port']) ?>
                    | Device ID: <?= Html::encode($analizorConfig['device_id']) ?>
                    | <?= Html::encode($analizorConfig['aciklama']) ?>
                </div>
                <?php else: ?>
                <div class="text-muted small mb-2">
                    <?= Html::encode($analizorConfig['aciklama']) ?>
                ...
                <div class="row" id="analizorCards">
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card border-success">
                            ...
                            <div class="card-body p-2">
                                <table class="table table-sm table-borderless mb-0">
                                    <tr><td class="text-muted">P L1</td><td class="text-right font-weight-bold" id="az_P_L1">---</td><td class="text-muted">W</td></tr>
                                    <tr><td class="text-muted">P L2</td><td class="text-right font-weight-bold" id="az_P_L2">---</td><td class="text-muted">W</td></tr>
                                    <tr><td class="text-muted">P L3</td><td class="text-right font-weight-bold" id="az_P_L3">---</td><td class="text-muted">W</td></tr>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <table class="table table-sm table-borderless mb-0">
                                            ...
                                            <tr><td class="text-muted">Toplam İhraç</td><td class="text-right font-weight-bold" id="az_E_export_total_kWh">---</td><td class="text-muted">kWh</td></tr>
                                        </table>
                                    </div>
                                    <div class="col-md-6">
                                        <table class="table table-sm table-borderless mb-0">
                                            <tr><td class="text-muted">Tüketim L1</td><td class="text-right" id="az_E_import_L1_kWh">---</td><td class="text-muted">kWh</td></tr>
                                            <tr><td class="text-muted">Tüketim L2</td><td class="text-right" id="az_E_import_L2_kWh">---</td><td class="text-muted">kWh</td></tr>
                                            <tr><td class="text-muted">Tüketim L3</td><td class="text-right" id="az_E_import_L3_kWh">---</td><td class="text-muted">kWh</td></tr>
                                        </table>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card h-100 border-secondary">
                            <div class="card-body d-flex flex-column align-items-center justify-content-center">
                                <p class="text-muted mb-2">Geçmiş ölçüm verileri</p>
                                <?= Html::a('📊 Geçmiş Verileri Görüntüle', ['analizor-gecmis', 'id' => $model->id], ['class' => 'btn btn-outline-primary']) ?>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            (function() {
                ...
                var apiUrl = '<?= Url::to(['ekipman/analizor-veri', 'id' => $model->id]) ?>';

                window.analizorToggle = function() {
                    if (polling) {
                        polling = false;
                        clearTimeout(timer);
                        document.getElementById('analizorStartBtn').innerHTML = '▶ Başlat';
                        document.getElementById('analizorStartBtn').className = 'btn btn-sm btn-outline-success ml-auto';
                        document.getElementById('analizorStatus').style.background = '#6c757d';
                        document.getElementById('analizorStatus').textContent = 'Durduruldu';
                    } else {
                        polling = true;
                        document.getElementById('analizorStartBtn').innerHTML = '■ Durdur';
                        document.getElementById('analizorStartBtn').className = 'btn btn-sm btn-outline-danger ml-auto';
                        analizorPoll();
                    }
                };

                function analizorPoll() {
                    if (!polling) return;
                    ...
                    fetch(apiUrl)
                        ...
                        .then(function(json) {
                            ...
                            if (json.success) {
                                ...
                                var fields = {
                                    ...
                                    'Freq': 2, 'I_avg_A': 1, 'I_N': 0,
                                    'PF_L1': 3, 'PF_L2': 3, 'PF_L3': 3, 'PF_avg': 3,
                                    'E_import_total_kWh': 1, 'E_export_total_kWh': 1,
                                    'E_import_L1_kWh': 1, 'E_import_L2_kWh': 1, 'E_import_L3_kWh': 1
                                };
                                for (var key in fields) {
                                    var el = document.getElementById('az_' + key);
                                    if (el && d[key] !== undefined && d[key] !== null) {
                                        el.textContent = parseFloat(d[key]).toFixed(fields[key]);
                                        el.style.color = '#333';
                                    } else if (el) {
                                        el.textContent = 'N/A';
                                        el.style.color = '#999';
                        ...
                        .catch(function(err) {
                            ...
                            document.getElementById('analizorTime').textContent = err.message;
                        })
                        .finally(function() {
                            if (polling) timer = setTimeout(analizorPoll, pollInterval);
                        });
                }
            })();
            </script>
